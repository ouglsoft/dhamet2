...
assert.match(shell, /2026-07-28-fresh-anonymous-v2/);
...
assert.match(online, /policyOverrides: \{ maxAdvancedPieces: 10 \}/);
...
const countedAbsence = MatchEnd.policyForEnd('opponent-absent', R.BOT, { policyOverrides: { maxAdvancedPieces: 10 } }, lateGame);
assert.equal(countedAbsence.countsAsResult, true, 'clear late absence may be adjudicated');
const earlyGame = { ...lateGame, ply: 10 };
const neutralAbsence = MatchEnd.policyForEnd('opponent-absent', R.BOT, { policyOverrides: { maxAdvancedPieces: 10 } }, earlyGame);
